...
(function () {
  ...
  /* ══════════════════════════════════════════
     3.5. DASHBOARD SIDEBAR TOGGLE
  ══════════════════════════════════════════ */
  const dashToggle = qs('#dash-sidebar-toggle');
  const dashSidebar = qs('.dash-sidebar');
  const dashOverlay = qs('#dash-overlay');
  ...
  document.addEventListener('click', e => {
    if (window.innerWidth <= 992 && dashSidebar?.classList.contains('open')) {
      if (!e.target.closest('.dash-sidebar') && !e.target.closest('#dash-sidebar-toggle')) {
        closeDashSidebar();
      }
    }
  });
  ...
  const navLinks = qsa('.nav-link[href^="#"], .mobile-nav-link[href^="#"]');

  function updateActiveNav() {
    let current = 'hero'; // Default to hero if none matched
    const scrollPos = window.scrollY + 150;

    sections.forEach(sec => {
      if (scrollPos >= sec.offsetTop) {
        // Only update current if the section ID is actually linked in the navbar
        if (qs(`.nav-link[href="#${sec.id}"]`)) {
          current = sec.id;
        }
      }
    });
  ...
  const revealObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        entry.target.classList.add('visible'); // keep for legacy support
        revealObserver.unobserve(entry.target);
      }
    });
  }, { threshold: 0.12 });

  qsa('.reveal, .reveal-scale').forEach(el => revealObserver.observe(el));
  ...
  /* ══════════════════════════════════════════
     12.  FAQ ACCORDION
  ══════════════════════════════════════════ */
  qsa('.faq-question, .acc-header').forEach(q => {
    q.addEventListener('click', () => {
      const item = q.closest('.faq-item, .acc-item');
      const isOpen = item.classList.contains('active') || item.classList.contains('open');

      qsa('.faq-item.open, .acc-item.active').forEach(i => {
        i.classList.remove('open');
        i.classList.remove('active');
      });

      if (!isOpen) {
        item.classList.add(item.classList.contains('acc-item') ? 'active' : 'open');
      }
    });
  });
  ...
  (function () {
    ...
    if (!el) return;
    const words = ['MASTER PLUMBERS', 'HYDRAULIC EXPERTS', 'SMART SOLUTIONS'];
  ...
  ══════════════════════════════════════════ */
  qsa('.product-card,.feature-card,.membership-card,.value-card,.service-card,.portal-card,.stat-card,.blog-card,.technician-card').forEach(card => {
